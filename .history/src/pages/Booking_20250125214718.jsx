...
const Booking = () => {
    ...
    const onSubmit = (data) => {
        ...
        // Prepare the bookings array (assuming `data` is an array of booking objects)
        const bookings = data.map((booking) => {
            const formData = new FormData();
    
            // Append all fields for each booking record
            formData.append("name", booking.name);
            formData.append("mobile", booking.phone);
            formData.append("fathers_name", booking.fathers_name);
            formData.append("mothers_name", booking.mothers_name);
            formData.append("address", booking.address);
            formData.append("nationality", booking.nationality);
            formData.append("profession", booking.profession);
            formData.append("company", booking.company);
            formData.append("comming_form", booking.comming_form);
            formData.append("purpose", booking.purpose);
            formData.append("checking_date_time", booking.date_time);
            formData.append("checkout_date_time", booking.checkout_date_time);
            formData.append("room_category", booking.room_category);
            formData.append("room_number", booking.room_number);
            formData.append("room_price", booking.room_price);
            formData.append("person", booking.person);
            formData.append("duration_day", booking.duration_day);
            formData.append("total_price", booking.total_price);
            formData.append("nid_no", booking.nid_no);
            formData.append("advance", booking.advance);
    
            // Append file fields if provided
            if (booking.nid_doc?.[0]) formData.append("nid_doc", booking.nid_doc[0]);
            if (booking.couple_doc?.[0]) formData.append("couple_doc", booking.couple_doc[0]);
            if (booking.passport_doc?.[0]) formData.append("passport_doc", booking.passport_doc[0]);
            if (booking.visa_doc?.[0]) formData.append("visa_doc", booking.visa_doc[0]);
            if (booking.other_doc?.[0]) formData.append("other_doc", booking.other_doc[0]);
    
            formData.append("passport_no", booking.passport_no);
            formData.append("visa_no", booking.visa_no);
            formData.append("payment_status", booking.payment_status);
            formData.append("payment_method", booking.payment_method);
            formData.append("booking_by", booking.booking_by);
    
            return formData; // Return FormData for each booking
        });
    
        // Send the bookings array as a JSON payload
        const payload = {
        ...
        axios
            .post(`${BASE_URL}/booking/add`, payload, {
                headers: {
