...
const Booking = () => {
    ...
    const onSubmit = (dataArray) => {
        ...
        dataArray.forEach((data, index) => {
            ...
            // Check if nid_doc is an array and append files accordingly
            if (Array.isArray(data.nid_doc)) {
                data.nid_doc.forEach((file, i) => {
                    formData.append(`bookings[${index}][nid_doc][${i}]`, file);
                });
            } else if (data.nid_doc) {
                // If it's a single file, append it directly
                formData.append(`bookings[${index}][nid_doc][0]`, data.nid_doc);
            }
    
            // Handle other document fields similarly
            if (Array.isArray(data.couple_doc)) {
                data.couple_doc.forEach((file, i) => {
                    formData.append(`bookings[${index}][couple_doc][${i}]`, file);
                });
            } else if (data.couple_doc) {
                formData.append(`bookings[${index}][couple_doc][0]`, data.couple_doc);
            }
    
            if (Array.isArray(data.passport_doc)) {
                data.passport_doc.forEach((file, i) => {
                    formData.append(`bookings[${index}][passport_doc][${i}]`, file);
                });
            } else if (data.passport_doc) {
                formData.append(`bookings[${index}][passport_doc][0]`, data.passport_doc);
            }
    
            if (Array.isArray(data.visa_doc)) {
                data.visa_doc.forEach((file, i) => {
                    formData.append(`bookings[${index}][visa_doc][${i}]`, file);
                });
            } else if (data.visa_doc) {
                formData.append(`bookings[${index}][visa_doc][0]`, data.visa_doc);
            }
    
            if (Array.isArray(data.other_doc)) {
                data.other_doc.forEach((file, i) => {
                    formData.append(`bookings[${index}][other_doc][${i}]`, file);
                });
            } else if (data.other_doc) {
                formData.append(`bookings[${index}][other_doc][0]`, data.other_doc);
            }
        });
