...
const Booking = () => {
    ...
    const onSubmit = (dataArray) => {
        ...
        dataArray.forEach((data, index) => {
            ...
            // Ensure nid_doc is always an array
            let nidDocFiles = Array.isArray(data.nid_doc) ? data.nid_doc : [data.nid_doc];
            nidDocFiles.forEach((file, i) => {
                if (file) {
                    formData.append(`bookings[${index}][nid_doc][${i}]`, file);
                }
            });
    
            // Do the same for other document fields
            let coupleDocFiles = Array.isArray(data.couple_doc) ? data.couple_doc : [data.couple_doc];
            coupleDocFiles.forEach((file, i) => {
                if (file) {
                    formData.append(`bookings[${index}][couple_doc][${i}]`, file);
                }
            });
    
            let passportDocFiles = Array.isArray(data.passport_doc) ? data.passport_doc : [data.passport_doc];
            passportDocFiles.forEach((file, i) => {
                if (file) {
                    formData.append(`bookings[${index}][passport_doc][${i}]`, file);
                }
            });
    
            let visaDocFiles = Array.isArray(data.visa_doc) ? data.visa_doc : [data.visa_doc];
            visaDocFiles.forEach((file, i) => {
                if (file) {
                    formData.append(`bookings[${index}][visa_doc][${i}]`, file);
                }
            });
    
            let otherDocFiles = Array.isArray(data.other_doc) ? data.other_doc : [data.other_doc];
            otherDocFiles.forEach((file, i) => {
                if (file) {
